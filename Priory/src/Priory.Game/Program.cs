...
static void SetupNewGame(GameEngine engine)
{
    ...
    var sex = PromptSex();
    engine.StartNewGame(playerName, sex);
}
...
static PlayerSex PromptSex()
{
    while (true)
    {
        Console.Write("Choose sex (M/F): ");
        var value = Console.ReadLine()?.Trim().ToLowerInvariant();
        if (value is "m" or "male") return PlayerSex.Male;
        if (value is "f" or "female") return PlayerSex.Female;
