...
public static class ServerHost
{
    public static Task RunAsync(string[] args, StoryData story, string saveRoot, SaveCodec saveCodec)
    {
        ...
        var accounts = new AccountRepository(saveRoot);
        ...
        app.MapPost("/api/sessions", (CreateSessionRequest request) =>
        {
            ...
            else
            {
                ...
                if (string.IsNullOrWhiteSpace(request.Password))
                    return Results.BadRequest(new { error = "password is required for new games" });

                if (!accounts.TryAuthenticate(request.PlayerName, request.Password, out _, out var authMessage))
                {
                    if (string.Equals(authMessage, "Unknown username.", StringComparison.Ordinal))
                    {
                        if (!accounts.Register(request.PlayerName, request.Password, request.PlayerName, out var registerMessage))
