...
public sealed class GameEngine
{
    ...
    private void AddContextTip(List<string> lines)
    {
        ...
        var actions = scene.Actions.Keys.OrderBy(x => x).Take(4).ToList();

        if (exits.Count > 0)
            lines.Add($"Available options: {string.Join(" | ", exits.Select(x => $"go {x}"))}.");

        if (actions.Count > 0)
            lines.Add($"You can also try: {string.Join(" | ", actions.Select(a => $"talk {a}"))}.");

        if (exits.Count == 0 && actions.Count == 0)
