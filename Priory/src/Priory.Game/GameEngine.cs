...
public sealed class GameEngine
{
    ...
    public PlayerOverview GetPlayerOverview()
    {
        ...
        return new PlayerOverview(_state.PlayerName, _state.LifePath, inventory, _state.Coin);
    }
    ...
    private void AddContextTip(List<string> lines)
    {
        ...
        var actionExamples = scene.Actions.Keys.Take(3).ToList();
        if (actionExamples.Count > 0)
            lines.Add($"Try interactions like: {string.Join(" | ", actionExamples.Select(a => $"talk to {a}"))}.");
        else
            lines.Add("Tip: use 'look', 'quests', or 'status' to reorient when no direct interaction is listed.");
    }
    ...
    private static IEnumerable<string> OpeningLoreBrief()
    {
        ...
        yield return "Saint Rose Priory endures, but its walls are weary: stores run thin, roofs strain, and rival factions watch your choices.";
    ...
    private static string LifePathFlavor(string optionText)
    {
        ...
        if (optionText.StartsWith("Scholar's Son", StringComparison.OrdinalIgnoreCase))
            return optionText + " (ink, argument, and memory)";
        if (optionText.StartsWith("Former Man-at-Arms", StringComparison.OrdinalIgnoreCase))
            return optionText + " (discipline, watchfulness, and scars)";
        ...
        if (optionText.StartsWith("Farmer's Son", StringComparison.OrdinalIgnoreCase))
            return optionText + " (soil, seasons, and endurance)";
        return optionText;
    }
    ...
    private void HandleMovement(string? target, List<string> lines)
    {
        ...
        {
            lines.Add("Go where? Try one of: " + string.Join(", ", scene.Exits.Keys));
            return;
        }
...
public sealed record PlayerOverview(string PlayerName, string? LifePath, List<string> Inventory, int Coin);
